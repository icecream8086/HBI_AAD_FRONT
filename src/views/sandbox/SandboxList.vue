<template>
  <div class="page-container">
      ...
      >
        <el-form-item :label="$t('sandbox.filterStatus')">
          <el-select
            ...
            clearable
            :placeholder="$t('sandbox.filterAll')"
            style="width:140px"
          >
            <el-option
              :label="$t('sandbox.statusRunning')"
              value="Running"
            />
            <el-option
              :label="$t('sandbox.statusStopped')"
              value="Stopped"
            />
            <el-option
              :label="$t('sandbox.statusPending')"
              value="Pending"
            />
            <el-option
              :label="$t('sandbox.statusFailed')"
              value="Failed"
            />
            <el-option
              :label="$t('sandbox.statusTerminated')"
              value="Terminated"
            />
          </el-select>
        </el-form-item>
    ...
    >
      ...
      >
        <template #default="{ row }">
          {{ fmtCell(row.createdAt, sandboxColumns.find(c => c.prop === 'createdAt')) }}
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
          <el-button
            size="small"
            @click="$router.push(`/sandboxes/${row.id}`)"
          >
            {{ $t('table.detail') }}
          </el-button>
          <el-button
      ...
      >
        <el-form-item :label="$t('sandbox.template')">
          <el-select
            ...
            filterable
            :placeholder="$t('sandbox.selectTemplate')"
            style="width:100%"
          >
            <el-option
        ...
        <el-form-item
          v-for="(c, i) in createForm.containers"
          :key="i"
          :label="`${$t('sandbox.containerLabel')} ${i+1}`"
        >
          <el-input
            v-model="c.image"
            :placeholder="$t('table.image')"
          ...
          <el-button
            ...
            size="small"
            circle
            @click="createForm.containers.splice(i,1)"
          >
            −
          </el-button>
        </el-form-item>
        <el-form-item>
          <el-button
            size="small"
            @click="createForm.containers.push({name:'',image:''})"
          >
            {{ $t('sandbox.addContainer') }}
          </el-button>
        </el-form-item>
      </el-form>
      <template #footer>
...
const { fmtCell } = useEntityColumns(sandboxColumns)
