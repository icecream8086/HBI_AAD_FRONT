<template>
  <div v-loading="loading">
    <el-button
      text
      class="back"
      @click="$router.push('/sandboxes/pods')"
    >
      {{ $t('pod.backToList') }}
    </el-button>
    ...
    <div v-if="pod">
      ...
      >
        <el-descriptions-item
          ...
          :span="3"
        >
          <code>{{ pod.providerId }}</code>
        </el-descriptions-item>
        <el-descriptions-item :label="$t('pod.platform')">
          {{ pod.platform || '-' }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('pod.region')">
          {{ pod.region || '-' }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('table.status')">
          <el-tag
            :type="statusType(pod.status)"
            size="small"
            effect="dark"
          >
            {{ pod.status }}
      ...
      >
        ...
        >
          <el-descriptions-item label="External IP">
            {{ pod.network.externalIp || '-' }}
          </el-descriptions-item>
          <el-descriptions-item label="Internal IP">
            {{ pod.network.internalIp || '-' }}
          </el-descriptions-item>
        </el-descriptions>
      </el-card>
      ...
      <el-card class="section">
        <template #header>
          {{ $t('pod.containers') }} ({{ pod.containers?.length || 0 }})
        </template>
        <div
        ...
        >
          ...
          >
            ...
            <el-descriptions-item :label="$t('pod.ready')">
              <el-tag
                :type="c.ready ? 'success' : 'danger'"
                size="small"
              >
                {{ c.ready ? 'Yes' : 'No' }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item :label="$t('pod.restartCount')">
              {{ c.restartCount ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item :label="$t('pod.startedAt')">
              {{ fmt(c.startedAt) }}
            </el-descriptions-item>
          </el-descriptions>
        </div>
        <el-empty
          v-if="!pod.containers?.length"
          :description="$t('pod.noContainers')"
          :image-size="50"
        />
      </el-card>
        ...
        >
          ...
          >
            <template #default="{ row }">
              <el-tag
                :type="row.status === 'True' ? 'success' : 'warning'"
                size="small"
              >
                {{ row.status }}
              </el-tag>
          ...
          />
          <el-table-column
            :label="$t('pod.startedAt')"
            width="170"
          >
            <template #default="{ row }">
              {{ fmt(row.lastTransitionTime) }}
            </template>
          </el-table-column>
        </el-table>
      </el-card>
      ...
      >
        ...
        >
          ...
          >
            <template #default="{ row }">
              {{ fmt(row.lastTimestamp) }}
            </template>
          </el-table-column>
        </el-table>
        <el-empty
          v-if="!pod.events?.length"
          :description="$t('pod.noEvents')"
          :image-size="50"
        />
      </el-card>
    </div>
