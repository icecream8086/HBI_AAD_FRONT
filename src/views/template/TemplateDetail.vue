<template>
  <div v-loading="loading">
    ...
    <div v-if="template">
      ...
      >
        ...
        <el-descriptions-item label="Creator">
          {{ userName(template.creatorId) }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('template.apiVersion')">
          <el-tag size="small">
            {{ template.apiVersion || 'hbi-aad/v1' }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item :label="$t('template.kind')">
          <el-tag
            :type="(template.kind || 'Container') === 'ContainerGroup' ? 'warning' : ''"
            size="small"
          >
        ...
        <el-descriptions-item :label="$t('table.createdAt')">
          {{ fmt(template.createdAt) }}
        </el-descriptions-item>
        <el-descriptions-item label="Platform">
          <el-tag>{{ template.container?.account || '-' }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="Region">
          {{ template.podSpec?.region || template.container?.region || '-' }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('topology.instanceTitle')">
          {{ template.container?.instanceId || '-' }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('topology.zone')">
          {{ template.container?.zone || '-' }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('table.restartPolicy')">
          {{ template.container?.restartPolicy || '-' }}
        </el-descriptions-item>
        <el-descriptions-item
        ...
        >
          <template v-if="template.instanceLimit">
            <el-tag size="small">
              {{ fmtLimitType(template.instanceLimit.type) }}
            </el-tag>
      ...
      <el-card class="section">
        <template #header>
          {{ $t('template.dagInherit') }} Chain
        </template>
        <div
          v-if="template.dependsOn?.length"
          class="dag-chain"
        >
          <el-tag
            v-for="(depId, i) in template.dependsOn"
            :key="depId"
            class="dag-node"
          >
            <span
          ...
          <p class="muted">
            {{ $t('template.inheritDescription') }}
          </p>
        </div>
        <el-empty
          v-else
          :description="$t('template.rootNode')"
          :image-size="50"
        />
      </el-card>
      ...
      <el-card class="section">
        <template #header>
          {{ $t('template.containers') }} ({{ resolvedContainers.length }})
        </template>
        <div
          v-for="(c, ci) in resolvedContainers"
          :key="ci"
          class="cont-detail"
        >
          <h4 class="cont-title">
            {{ c.name || `${$t('template.containerLabel')} ${ci+1}` }}
          </h4>
          <el-descriptions
            :column="4"
            border
          ...
          >
            <el-col :span="12">
              ...
              >
                ...
                <span
                  v-if="c.resources.limits.gpu"
                  style="margin-left:6px"
                >GPU: {{ c.resources.limits.gpu }}{{ c.resources.limits.gpuType ? '×'+c.resources.limits.gpuType : '' }}</span>
        ...
        <el-empty
          v-if="!resolvedContainers.length"
          :description="$t('template.noContainers')"
          :image-size="50"
        />
      </el-card>
      ...
      <!-- PodSpec (v2) -->
      <el-card
        v-if="template.kind === 'ContainerGroup' && template.podSpec"
        class="section"
      >
        <template #header>
          {{ $t('template.containerGroup') }}: {{ template.podSpec.name }}
        </template>
        <el-descriptions
          :column="3"
          border
          size="small"
        >
          <el-descriptions-item :label="$t('template.podRegion')">
            {{ template.podSpec.region || '-' }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('template.podCpu')">
            {{ template.podSpec.resources?.cpu || '-' }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('template.podMemory')">
            {{ template.podSpec.resources?.memory || '-' }}
          </el-descriptions-item>
        </el-descriptions>
        <div
          v-for="(svcDef, svcName) in template.podSpec.services"
          :key="svcName"
          class="cont-detail"
        ...
        >
          ...
          >
            <el-col :span="12">
              <div
                v-if="svcDef.ports?.length"
                class="sub-section"
              >
                <h5>{{ $t('template.portMapping') }}</h5>
                <el-tag
                  v-for="(p, pi) in svcDef.ports"
                  :key="pi"
                  size="small"
                  style="margin-right:4px"
                >
            ...
            <el-col :span="12">
              <div
                v-if="svcDef.dependsOn?.length"
                class="sub-section"
              >
                <h5>{{ $t('template.serviceDepends') }}</h5>
                <el-tag
                  v-for="(dep, di) in svcDef.dependsOn"
                  :key="di"
                  size="small"
                  style="margin-right:4px"
                >
              ...
              <div
                v-if="svcDef.env?.length"
                class="sub-section"
              >
                <h5>{{ $t('table.env') }}</h5>
                <el-tag
                  v-for="(e, ei) in svcDef.env"
                  :key="ei"
                  size="small"
                  style="margin-right:4px"
                >
      ...
      >
        ...
        <el-descriptions
          ...
          size="small"
        >
          <el-descriptions-item :label="$t('template.mode')">
            {{ template.network.mode || '-' }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('template.publicIp')">
            {{ template.network.publicIp?.allocate ? $t('template.publicIpAllocated') : $t('template.publicIpNotAllocated') }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('template.vpcId')">
            {{ template.network.vpc?.id || '-' }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('template.securityGroup')">
            {{ template.network.vpc?.securityGroupId || '-' }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('template.ipAddress')">
            {{ template.network.ipAddress || '-' }}
          </el-descriptions-item>
        </el-descriptions>
      </el-card>
      ...
      >
        ...
        <div
          v-if="template.extensions.storage?.length"
          class="storage-list"
        >
          <div
            v-for="(st, si) in template.extensions.storage"
            :key="si"
            class="storage-card"
          >
        ...
        <pre
          v-else
          class="json-block"
        >{{ JSON.stringify(template.extensions, null, 2) }}</pre>
      </el-card>
      <!-- Health Checks -->
      <el-card
        v-if="template.healthChecks?.length"
        class="section"
      >
        <template #header>
          {{ $t('template.healthChecks') }} ({{ template.healthChecks.length }})
        </template>
        <div
          v-for="(hc, hi) in template.healthChecks"
          :key="hi"
          class="cont-box"
    ...
    <el-empty
      v-else-if="!loading"
      :description="$t('template.notFound')"
    />
    ...
    >
      ...
      >
        ...
        <el-form-item :label="$t('template.templateType')">
          <el-radio-group v-model="edit.form.templateType">
            <el-radio value="Container">
              {{ $t('template.kindContainer') }}
            </el-radio>
            <el-radio value="ContainerGroup">
              {{ $t('template.kindContainerGroup') }}
            </el-radio>
          </el-radio-group>
        </el-form-item>
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item :label="$t('template.singleton')">
              <el-select
                v-model="edit.form.limitType"
                clearable
                :placeholder="$t('template.singletonHint')"
                style="width:100%"
              >
                <el-option
                  :label="$t('template.limitFixed')"
                  value="fixed"
                />
                <el-option
                  :label="$t('template.limitPerUser')"
                  value="perUser"
                />
                <el-option
                  :label="$t('template.limitPerSystem')"
                  value="perSystem"
                />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item :label="$t('table.limit')">
              <el-input-number
                v-model="edit.form.limitMax"
                :min="1"
                :max="999"
                style="width:100%"
              />
          ...
          <el-row :gutter="12">
            ...
            <el-col :span="12">
              <el-form-item :label="$t('template.restartPolicy')">
                <el-select
                  v-model="edit.form.restartPolicy"
                  clearable
                >
                  <el-option
                    label="Always"
          ...
          <el-row :gutter="12">
            ...
            <el-col :span="12">
              <el-form-item :label="$t('topology.zone')">
                <el-input
                  v-model="edit.form.zone"
                  placeholder="cn-hangzhou-a"
                />
              </el-form-item>
            </el-col>
          </el-row>
        ...
        <template v-if="edit.form.templateType === 'ContainerGroup'">
          ...
          <el-row :gutter="12">
            <el-col :span="8">
              <el-form-item :label="$t('template.podName')">
                <el-input
                  v-model="edit.form.podName"
                  placeholder="Container group name"
                />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item :label="$t('template.podRegion')">
                <el-input
                  v-model="edit.form.podRegion"
                  placeholder="cn-hangzhou"
                />
              </el-form-item>
            </el-col>
          ...
          <el-row :gutter="12">
            <el-col :span="8">
              <el-form-item :label="$t('template.podCpu')">
                <el-input
                  v-model="edit.form.podCpu"
                  placeholder="e.g. 1"
                />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item :label="$t('template.podMemory')">
                <el-input
                  v-model="edit.form.podMemory"
                  placeholder="e.g. 2Gi"
                />
              </el-form-item>
            </el-col>
          ...
          <div
            v-for="(s, si) in edit.form.services"
            :key="si"
            class="cont-card"
          >
            <el-form-item :label="`${$t('template.serviceName')} ${si+1}`">
              ...
              <el-input
                v-model="s.image"
                :placeholder="$t('template.imagePlaceholder')"
              ...
              <el-button
                ...
                size="small"
                circle
                @click="edit.form.services.splice(si,1)"
              >
                −
              </el-button>
            </el-form-item>
            ...
            <el-form-item :label="$t('template.port')">
              ...
              <el-button
                size="small"
                @click="s.ports.push({containerPort:80,protocol:'TCP'})"
              >
                {{ $t('template.addPort') }}
              </el-button>
            </el-form-item>
            ...
            <el-form-item :label="$t('template.serviceDepends')">
              <el-select
                ...
                multiple
                filterable
                :placeholder="$t('template.dependsPlaceholder')"
                style="width:100%"
                size="small"
              >
                <el-option
                  v-for="other in edit.form.services.filter((_, oi) => oi !== si)"
                  :key="other.name"
                  :label="other.name"
                  :value="other.name"
                />
            ...
            <el-form-item :label="$t('template.env')">
              ...
              <el-button
                size="small"
                @click="s.env.push({key:'',value:''})"
              >
                {{ $t('template.addEnvVar') }}
              </el-button>
            </el-form-item>
          </div>
          <el-form-item>
            <el-button
              size="small"
              @click="edit.form.services.push(emptyService())"
            >
              {{ $t('template.addService') }}
            </el-button>
          </el-form-item>
        </template>
        ...
        <template v-if="edit.form.templateType === 'Container'">
          ...
          <div
            v-for="(c, ci) in edit.form.containers"
            :key="ci"
            class="cont-card"
          >
            <el-form-item :label="`#${ci+1}`">
              ...
              <el-button
                ...
                size="small"
                circle
                @click="edit.form.containers.splice(ci,1)"
              >
                −
              </el-button>
            </el-form-item>
            ...
            <el-form-item :label="$t('template.port')">
              ...
              <el-button
                size="small"
                @click="c.ports.push({containerPort:80,protocol:'TCP'})"
              >
                {{ $t('template.addPort') }}
              </el-button>
            </el-form-item>
        ...
        <el-row :gutter="12">
          <el-col :span="8">
            <el-form-item :label="$t('template.mode')">
              <el-select
                v-model="edit.form.networkMode"
                clearable
                :placeholder="$t('template.singletonHint')"
                style="width:100%"
              >
        ...
        <el-form-item :label="$t('template.securityGroup')">
          <el-select
            v-model="edit.form.securityGroupId"
            filterable
            clearable
            placeholder="Select security group ID"
            style="width:100%"
          >
            <el-option
              v-for="sg in sgList"
        ...
        <div
          v-if="edit.form.networkMode === 'vpc'"
          class="cont-card"
        >
          <el-form-item
            :label="$t('template.subnetIds')"
            label-width="120px"
          >
            <el-select
              v-model="edit.form.subnetIds"
              filterable
              allow-create
              clearable
              multiple
        ...
        <div
          v-for="(st, si) in edit.form.storage"
          :key="si"
          class="cont-card"
        >
          <el-form-item :label="`${$t('table.volume')} ${si+1}`">
            ...
            <el-button
              type="danger"
              size="small"
              @click="edit.form.storage.splice(si,1)"
            >
              ✕
            </el-button>
          </el-form-item>
            ...
            <el-col :span="24">
              ...
              >
                <el-select
                  ...
                  size="small"
                  style="width:100%"
                  @change="onBucketChange(st)"
                >
                  <el-option
                    v-for="b in buckets"
                    :key="b.id"
                    :label="`${b.name} (${b.bucketType})`"
                    :value="b.id"
                  />
                </el-select>
              </el-form-item>
        ...
        <div class="mode-toggle">
          <el-button
            size="small"
            text
            @click="toggleHcJsonMode"
          >
            {{ edit.healthChecksJsonMode ? '表单模式' : 'JSON 模式' }}
          </el-button>
        </div>
        <template v-if="!edit.healthChecksJsonMode">
          <div
            v-for="(hc, hi) in edit.form.healthChecks"
            :key="hi"
            class="cont-card"
          >
            <el-row :gutter="8">
              ...
              <el-col :span="6">
                ...
                >
                  ...
                  >
                    <el-option
                      v-for="ct in edit.form.containers"
                      :key="ct.name"
                      :label="'container:'+ct.name"
                      :value="'container:'+ct.name"
                    />
                  </el-select>
                </el-form-item>
              </el-col>
              ...
              <el-col :span="2">
                <el-button
                  ...
                  size="small"
                  circle
                  @click="edit.form.healthChecks.splice(hi,1)"
                >
                  −
                </el-button>
              </el-col>
            ...
            <el-row :gutter="8">
              ...
              <el-col :span="4">
                ...
                >
                  <el-input-number
                    v-model="hc.initialDelaySeconds"
                    :min="0"
                    size="small"
                    style="width:100%"
                  />
          ...
          <el-button
            size="small"
            style="margin-top:8px"
            @click="edit.form.healthChecks.push(emptyHc())"
          >
            {{ $t('template.addHealthCheck') }}
          </el-button>
        </template>
        <template v-else>
          <el-input
            v-model="edit.healthChecksJsonStr"
            type="textarea"
            :rows="8"
            placeholder="[{&quot;name&quot;:&quot;...&quot;,&quot;target&quot;:&quot;container:...&quot;,&quot;type&quot;:&quot;readiness&quot;,&quot;probe&quot;:{&quot;tcpSocket&quot;:{&quot;port&quot;:80}}}]"
          />
        </template>
        <el-divider>{{ $t('template.extraParams') }}</el-divider>
        <el-form-item :label="$t('template.healthRetry')">
          <el-input-number
            v-model="edit.form.healthMaxRetries"
            :min="-1"
            :step="1"
            style="width:100%"
          />
        ...
        <el-form-item :label="$t('template.vendorPassthrough')">
          <el-input
            v-model="edit.form.providerOverridesStr"
            type="textarea"
            :rows="2"
            placeholder="{&quot;eipBandwidth&quot;:100}"
          />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="edit.show=false">
          {{ $t('template.cancel') }}
        </el-button>
        <el-button
          type="primary"
    ...
    >
      ...
      >
        ...
        >
          <el-select
            v-model="applyDlg.form.instanceId"
            filterable
            :placeholder="$t('topology.instanceTitle')"
            style="width:100%"
          >
            <el-option
              v-for="inst in instances"
      ...
      <template #footer>
        <el-button @click="applyDlg.show = false">
          {{ $t('table.cancel') }}
        </el-button>
        <el-button
          type="primary"
          :loading="applyDlg.saving"
...
async function saveGroupName() {
  ...
  try {
    if (template.value.kind === 'ContainerGroup') {
      ...
      await api.templates.update(template.value.id, { podSpec })
    } else {
      const containers = [...(template.value.container?.containers || [])]
      if (containers.length > 0) {
        containers[0] = { ...containers[0], name: newName }
        await api.templates.update(template.value.id, { container: { containers } })
      }
    }
...
function hcsFromSpec(hcs?: HealthCheckDef[]): HealthCheckForm[] { return (hcs || []).map(hcToForm) }
function buildHcSpec(hc: HealthCheckForm): HealthCheckDef {
...
// ---- Service codec (ServiceForm ↔ ServiceDef) ----
function serviceToForm(name: string, raw: ServiceDef): ServiceForm {
  return {
    name, image: raw?.image || '', command: raw?.command ? JSON.stringify(raw.command) : '',
    ports: (raw?.ports || []).map(p => ({ containerPort: p.containerPort ?? 80, protocol: p.protocol || 'TCP' })),
    cpu: raw?.resources?.cpu || '', memory: raw?.resources?.memory || '',
    dependsOn: raw?.dependsOn || [],
    env: (raw?.env || []).map(e => ({ key: e.name || '', value: e.value || '' })),
  }
}
...
function buildServiceSpec(s: ServiceForm): ServiceDef {
  const def: ServiceDef = { image: s.image, resources: { cpu: '', memory: '' } }
...
// ---- Container codec (ContForm ↔ ContainerDef) ----
function contToForm(raw: ContainerDef): ContForm {
  return {
...
function contsFromSpec(containers?: ContainerDef[]): ContForm[] { return (containers || []).map(c => contToForm(c)) }
function buildContSpec(c: ContForm): ContainerDef {
  const ct: ContainerDef = { name: c.name, image: c.image }
  ...
  if (c.cpu && ct.resources) ct.resources.limits = { ...ct.resources.limits, cpu: c.cpu }
  if (c.memory && ct.resources) ct.resources.limits = { ...ct.resources.limits, memory: c.memory }
  if (c.gpu && ct.resources) ct.resources.limits = { ...ct.resources.limits, gpu: c.gpu }
  if (c.gpuType && ct.resources) ct.resources.limits = { ...ct.resources.limits, gpuType: c.gpuType }
...
async function handleSave() {
  ...
  edit.saving = true
  try {
    const body: Record<string, unknown> = { name: edit.form.name }
    ...
    if (edit.form.templateType === 'ContainerGroup') {
      ...
      if (svcs.length) {
        const serviceMap: Record<string, ServiceDef> = {}
        svcs.forEach(s => { serviceMap[s.name] = buildServiceSpec(s) })
        podSpec.services = serviceMap
      }
      if (Object.keys(podSpec).length) body.podSpec = podSpec as PodSpec
    } else {
      ...
      if (containers.length || edit.form.region || edit.form.restartPolicy || edit.form.instanceId || edit.form.zone) {
        const container: Partial<ContainerSpec> & { containers: ContainerDef[] } = { containers }
        ...
        body.container = container as ContainerSpec
      }
    }
    const net: Partial<TemplateNetworkSpec> = {}
    const vpc: Record<string, string | string[]> = {}
    if (edit.form.networkMode) net.mode = edit.form.networkMode as TemplateNetworkSpec['mode']
    ...
    if (Object.keys(vpc).length) net.vpc = vpc as TemplateNetworkSpec['vpc']
    if (Object.keys(net).length) body.network = net as TemplateNetworkSpec
    const extensions: Partial<TemplateExtensions> = {}
    ...
    if (storage.length) extensions.storage = storage.map(s => {
      const item: Partial<TemplateStorage> = { name: s.name, mountPath: s.mountPath, type: s.type === 'volume' ? 'nfs' : 'oss' }
      if (s.volumeId) item.volumeId = s.volumeId
      if (s.bucketId) item.bucketId = s.bucketId
      if (s.instanceId) item.instanceId = s.instanceId
      return item as TemplateStorage
    })
    if (Object.keys(extensions).length) body.extensions = extensions as TemplateExtensions
    ...
    await api.templates.update(route.params.id as string, body)
...
async function handleApplyConfirm() {
  ...
  try {
    const body: Record<string, unknown> = { name: applyDlg.form.name, instanceId: applyDlg.form.instanceId }
    await api.templates.apply(route.params.id as string, body)
...
onMounted(async () => {
  ...
  try { volumes.value = await api.topology.volumes.list().then(r => (r.items ?? []) as Volume[]) } catch { /* ignore */ }
  try { buckets.value = await api.topology.buckets.list().then(r => (r.items ?? []) as RegionBucket[]) } catch { /* ignore */ }
