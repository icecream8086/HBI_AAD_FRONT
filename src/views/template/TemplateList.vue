<template>
  <div>
    ...
    >
      ...
      />
      <el-table-column
        :label="$t('template.kind')"
        width="150"
      >
        <template #default="{ row }">
          <el-tag
            :type="(row.kind || 'Container') === 'ContainerGroup' ? 'warning' : ''"
            size="small"
          >
      ...
      <el-table-column
        :label="$t('template.containerLabel')"
        width="100"
      >
        <template #default="{ row }">
          <template v-if="row.kind === 'ContainerGroup'">
            {{ Object.keys(row.podSpec?.services || {}).length || 0 }}
          </template>
          <template v-else>
            {{ row.container?.containers?.length || 0 }}
          </template>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
          {{ row.podSpec?.region || row.container?.region || '-' }}
        </template>
      </el-table-column>
      <el-table-column
        :label="$t('table.restartPolicy')"
        width="100"
      >
        <template #default="{ row }">
          {{ row.container?.restartPolicy || '-' }}
        </template>
      </el-table-column>
      <el-table-column
        :label="$t('table.limit')"
      ...
      >
        <template #default="{ row }">
          <template v-if="row.instanceLimit">
            <el-tag size="small">
              {{ fmtLimitType(row.instanceLimit.type) }}
            </el-tag> :{{ row.instanceLimit.max }}
          </template>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column
        :label="$t('table.dagDep')"
        width="160"
        show-overflow-tooltip
      >
        <template #default="{ row }">
          {{ row.dependsOn?.map(id => templateName(id)).join(', ') || '-' }}
        </template>
      </el-table-column>
      <el-table-column
    ...
    <el-dialog
      v-model="dlg.show"
      :title="$t('template.createTitle')"
      width="860px"
      :close-on-click-modal="false"
    >
      <el-form
        v-loading="dlg.saving"
        :model="f"
        label-width="100px"
      >
        <el-form-item :label="$t('template.name')">
        ...
        <el-form-item v-if="f.templateType === 'ContainerGroup'">
          ...
          <span
            class="hint"
            style="margin-left:6px;margin-top:0"
          >{{ $t('template.demoPodDesc') }}</span>
        </el-form-item>
        ...
        <template v-if="f.templateType === 'Container'">
          ...
          <el-row :gutter="12">
            <el-col :span="8">
              <el-form-item :label="$t('template.platform')">
                <el-select
                  v-model="f.account"
                  filterable
                  allow-create
        ...
        <div
          ...
          class="inherited-box"
        >
          <p class="inherited-title">
            {{ $t('table.inheritedReadonly') }}
          </p>
          <el-tag
            v-for="(c, i) in inherited"
            :key="i"
            class="inherited-tag"
            type="info"
          >
        ...
        <template v-if="f.templateType === 'ContainerGroup'">
          ...
          >
            ...
            <el-form-item :label="$t('template.serviceDepends')">
              ...
              >
                <el-option
                  v-for="other in f.services.filter((_, oi) => oi !== si)"
                  :key="other.name"
                  :label="other.name"
                  :value="other.name"
                />
          ...
          >
            <el-form-item :label="`${$t('template.containerLabel')} ${ci+1}`">
              ...
              <el-input
                v-model="c.image"
                :placeholder="$t('template.imagePlaceholder')"
                style="width:320px;margin-right:6px"
                size="small"
              />
              <el-button
                type="danger"
                size="small"
                circle
                @click="f.containers.splice(ci,1)"
              >
                −
              </el-button>
            </el-form-item>
        ...
        <el-row :gutter="12">
          <el-col :span="8">
            <el-form-item :label="$t('template.publicIp')">
              <el-switch v-model="f.allocatePublicIp" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item :label="$t('template.healthRetry')">
              <el-input-number
                v-model="f.healthMaxRetries"
                :min="-1"
                :step="1"
                style="width:100%"
              />
        ...
        <el-form-item :label="$t('template.vendorPassthrough')">
          <el-input
            v-model="f.providerOverridesStr"
            type="textarea"
            :rows="2"
          ...
          <el-row :gutter="8">
            ...
            <el-col :span="2">
              <el-button
                ...
                size="small"
                circle
                @click="f.healthChecks.splice(hi,1)"
              >
                −
              </el-button>
            </el-col>
        ...
        <el-button
          ...
          @click="f.healthChecks.push(emptyHc())"
        >
...
function specFromForm() {
  ...
  if (f.templateType === 'ContainerGroup') {
    const podSpec: Partial<PodSpec> = { name: f.podName || f.name }
    ...
    if (svcs.length) {
      ...
      svcs.forEach(s => {
        ...
        serviceMap[s.name] = def
      })
      podSpec.services = serviceMap
    }
    if (Object.keys(podSpec).length > 1 || podSpec.services) body.podSpec = podSpec as PodSpec
  } else {
    const container: Partial<ContainerSpec> & { containers?: ContainerDef[] } = {}
    ...
    if (container.containers?.length) body.container = container as ContainerSpec
  }
...
async function handleSave() {
  ...
  dlg.saving = true
  try {
    const body: Record<string, unknown> = { name: f.name, ...(specFromForm() || {}) }
