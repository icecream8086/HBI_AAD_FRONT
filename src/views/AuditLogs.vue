<template>
  <div>
    ...
    <el-card class="filters">
      ...
      >
        ...
        <el-form-item :label="$t('audit.facility')">
          <el-input
            v-model="f.facility"
            :placeholder="$t('audit.facilityPlaceholder')"
            style="width:150px"
            clearable
          />
        </el-form-item>
    ...
    >
      ...
      >
        <template #default="{ row }">
          {{ userName(row.actorId) }}
        </template>
      </el-table-column>
      <el-table-column
        prop="message"
        :label="$t('audit.message')"
        min-width="260"
        show-overflow-tooltip
      />
      <el-table-column
      ...
      <template v-if="detail.item">
        ...
        >
          ...
          <el-descriptions-item :label="$t('audit.time')">
            {{ fmt(detail.item.timestamp) }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('audit.level')">
            <el-tag
              :type="levelType(detail.item.level)"
              size="small"
            >
              {{ levelName(detail.item.level) }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item :label="$t('audit.facility')">
            {{ detail.item.facility }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('audit.requestId')">
            <code v-if="detail.item.requestId">{{ detail.item.requestId }}</code>
            <span v-else>-</span>
          </el-descriptions-item>
          <el-descriptions-item :label="$t('audit.actor')">
            {{ userName(detail.item.actorId) }}
          </el-descriptions-item>
          <el-descriptions-item
            v-if="detail.item.actorId"
            :label="$t('audit.actorIdRaw')"
          >
        ...
        <el-collapse
          v-if="detail.item.metadata && Object.keys(detail.item.metadata).length"
