  ...
  <div>
    ...
    >
      <el-form @submit.prevent="handleCreate">
        ...
        >
          <el-input
            v-model="formImage"
            :placeholder="$t('image.pullPlaceholder')"
            @keyup.enter="handleCreate"
          />
        </el-form-item>
        <el-form-item :label="$t('topology.credentialRef')">
          <el-select
            v-model="formCredentialRef"
            filterable
            clearable
            placeholder="Optional — 选择已有凭证"
            style="width:100%"
          >
            <el-option
              v-for="c in pullCreds"
              :key="c.name"
          ...
          <div
            class="hint inline-cred-toggle"
            @click="showInlineCred = !showInlineCred"
          >
            {{ showInlineCred ? '−' : '+' }} {{ $t('image.inlineCred') }}
          </div>
          <template v-if="showInlineCred">
            <el-input
              v-model="formRegistryServer"
      ...
      <template #footer>
        <el-button @click="showPull=false">
          {{ $t('table.cancel') }}
        </el-button>
        <el-button
          type="primary"
    ...
    >
      ...
      >
        ...
        <el-descriptions-item :label="$t('image.imageCol')">
          <code>{{ detail.image }}</code>
        </el-descriptions-item>
        <el-descriptions-item
          v-if="detail.credentialRef"
...
const imageStatusTags: StatusTagMap<ImageStatus> = {
  pending: 'primary',
  pulling: 'warning',
  ready: 'success',
  error: 'danger',
}
function statusType(s: string) { return lookup(imageStatusTags, s, 'danger') }
