<template>
  <div>
    ...
    <el-table
      ...
      :data="items"
      stripe
      :empty-text="$t('action.noSecrets')"
    >
      <el-table-column
        prop="key"
        :label="$t('action.secretName')"
        min-width="180"
      />
      <el-table-column
        :label="$t('action.secretValue')"
        width="120"
      >
        <template #default>
          {{ $t('action.valueMasked') }}
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
          {{ row.createdAt ? fmt(row.createdAt) : '-' }}
        </template>
      </el-table-column>
      <el-table-column
    ...
    <el-dialog
      v-model="dlg.show"
      :title="$t('action.createSecretTitle')"
      width="450px"
      destroy-on-close
    >
      <el-form
      ...
      >
        ...
        >
          <el-input
            ...
            :placeholder="$t('action.secretNamePlaceholder')"
          />
        </el-form-item>
        <el-form-item
          :label="$t('action.secretValue')"
