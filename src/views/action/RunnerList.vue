<template>
  <div>
    ...
    >
      <el-table-column
        prop="name"
        :label="$t('action.runnerName')"
        min-width="140"
      />
      <el-table-column
        :label="$t('action.runnerStatus')"
        width="100"
      >
        <template #default="{ row }">
          <el-tag
            :type="runnerStatusType(row.status)"
            size="small"
          >
            {{ $t(`action.status${row.status.charAt(0).toUpperCase()}${row.status.slice(1)}`) || row.status }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column
        :label="$t('action.runnerLabels')"
        min-width="150"
      >
        <template #default="{ row }">
      ...
      <el-table-column
        :label="$t('action.runnerLastHeartbeat')"
        width="170"
      >
        <template #default="{ row }">
          {{ fmt(row.lastHeartbeatAt) }}
        </template>
      </el-table-column>
      <el-table-column
