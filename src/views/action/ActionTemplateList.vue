<template>
  <div>
    ...
    <el-card class="filters">
      <el-form inline>
        <el-form-item :label="$t('actionTemplate.category')">
          ...
          >
            <el-option
              :label="$t('actionTemplate.allCategories')"
              value=""
            />
            <el-option
              :label="$t('actionTemplate.ci')"
              value="ci"
            />
            <el-option
              :label="$t('actionTemplate.deploy')"
              value="deploy"
            />
            <el-option
              :label="$t('actionTemplate.service')"
              value="service"
            />
            <el-option
              :label="$t('actionTemplate.maintenance')"
              value="maintenance"
            />
            <el-option
              :label="$t('actionTemplate.test')"
              value="test"
            />
          </el-select>
        </el-form-item>
    ...
    >
      <el-table-column
        prop="name"
        :label="$t('actionTemplate.name')"
        min-width="160"
      />
      <el-table-column
        :label="$t('actionTemplate.category')"
        width="120"
      >
        <template #default="{ row }">
          <el-tag
            ...
            size="small"
          >
            {{ $t(`actionTemplate.${row.category}`) }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column
        :label="$t('actionTemplate.description')"
        min-width="200"
      ...
      >
        <template #default="{ row }">
          ...
          <span
            v-else
            class="dim"
          >{{ $t('actionTemplate.noTags') }}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="version"
        :label="$t('actionTemplate.version')"
        width="90"
      />
      <el-table-column
    ...
    <!-- Detail Dialog -->
    <el-dialog
      v-model="detailDlg.show"
      :title="$t('actionTemplate.detailTitle')"
      width="700px"
      destroy-on-close
    >
      <template v-if="detailDlg.template">
        ...
        >
          ...
          <el-descriptions-item :label="$t('actionTemplate.version')">
            {{ detailDlg.template.version || '-' }}
          </el-descriptions-item>
          <el-descriptions-item :label="$t('actionTemplate.category')">
            <el-tag
              :type="categoryType(detailDlg.template.category)"
              size="small"
            >
              {{ $t(`actionTemplate.${detailDlg.template.category}`) }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item
            :label="$t('actionTemplate.tags')"
          ...
          >
            <template v-if="detailDlg.template.tags && detailDlg.template.tags.length">
              <el-tag
                v-for="tag in detailDlg.template.tags"
                :key="tag"
                size="small"
                style="margin-right:4px"
              >
        ...
        <div
          v-if="detailDlg.template.trigger"
          class="trigger-section"
        >
          <el-tag
            v-if="detailDlg.template.trigger.manual"
            size="small"
            type="primary"
          >
          ...
          <el-tag
            v-else-if="detailDlg.template.trigger.cron"
            size="small"
            type="warning"
          >
            {{ $t('action.triggerCron') }}: {{ detailDlg.template.trigger.cron }}
          </el-tag>
          <el-tag
            v-else-if="detailDlg.template.trigger.webhook"
            size="small"
            type="success"
          >
            {{ $t('action.triggerWebhook') }}
          </el-tag>
          <el-tag
            v-else-if="detailDlg.template.trigger.http"
            size="small"
            type="success"
          >
        ...
        <template v-if="detailDlg.template.jobs && Object.keys(detailDlg.template.jobs).length">
          <el-table
            :data="jobList(detailDlg.template.jobs)"
            stripe
            size="small"
            :empty-text="$t('table.empty')"
          >
      ...
      <div
        v-else-if="detailDlg.loading"
        v-loading="detailDlg.loading"
        style="height:200px"
      />
      <template #footer>
        <el-button @click="detailDlg.show = false">
          {{ $t('table.close') }}
        </el-button>
      </template>
    </el-dialog>
