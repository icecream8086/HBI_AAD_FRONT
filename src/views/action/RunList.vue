<template>
  <div>
    ...
    <el-card class="filters">
      <el-form inline>
        <el-form-item :label="$t('action.runStatus')">
          <el-select v-model="filter.status" clearable :placeholder="$t('table.selectPlaceholder')" style="width:140px" @change="fetchData">
            <el-option label="Pending" value="Pending" />
            <el-option label="Running" value="Running" />
            <el-option label="Success" value="Success" />
            <el-option label="Failure" value="Failure" />
            <el-option label="Cancelled" value="Cancelled" />
            <el-option label="TimedOut" value="TimedOut" />
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="items" v-loading="loading" stripe :empty-text="$t('table.empty')">
      ...
      <el-table-column :label="$t('action.runTrigger')" width="110">
        <template #default="{ row }">
      ...
      <el-table-column :label="$t('action.runDuration')" width="100">
      ...
      <el-table-column :label="$t('table.actions')" width="240" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 'Running' || row.status === 'Pending'" size="small" type="warning" @click="handleCancel(row.id)">{{ $t('action.cancelRun') }}</el-button>
          <el-button v-if="row.status === 'Failure' || row.status === 'Cancelled' || row.status === 'TimedOut'" size="small" @click="handleRerun(row.id)">{{ $t('action.rerun') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
