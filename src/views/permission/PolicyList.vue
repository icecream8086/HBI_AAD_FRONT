<template>
  <div>
    ...
    <el-table :data="policies" v-loading="loading" stripe empty-text="暂无策略">
    ...
    <el-dialog v-model="dialog.show" :title="dialog.isEdit?'编辑策略':'新建策略'" width="550px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="资源">
          <el-input v-model="form.resource" placeholder="如: sandbox:* 或 template:123" />
        </el-form-item>
...
function openCreate() {
  ...
  form.name = ''; form.effect = 'allow'; form.actions = []; form.resource = ''; form.priority = 100
  dialog.show = true
}
function openEdit(row: StoredPolicy) {
  dialog.isEdit = true; dialog.editId = row.id
  form.name = row.name; form.effect = row.effect; form.actions = [...row.actions]; form.resource = row.resource || ''; form.priority = row.priority
  dialog.show = true
}

async function fetchData() {
  loading.value = true
  try { policies.value = await api.extractItems<StoredPolicy>(api.permissions.apiPermissionsPoliciesGet()) }
