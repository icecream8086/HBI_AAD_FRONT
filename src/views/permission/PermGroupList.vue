<template>
  <div>
    ...
    <div class="page-head"><h2>权限组</h2><el-button type="primary" size="small" @click="openCreate">新建权限组</el-button></div>
    <el-table :data="groups" v-loading="loading" stripe empty-text="暂无权限组">
      ...
      <el-table-column label="绑定用户组" width="150" show-overflow-tooltip><template #default="{ row }">{{ row.userGroupIds?.join(', ') || '-' }}</template></el-table-column>
      <el-table-column label="绑定用户" width="150" show-overflow-tooltip><template #default="{ row }">{{ row.userIds?.join(', ') || '-' }}</template></el-table-column>
      <el-table-column prop="dependsOn" label="依赖" width="200" show-overflow-tooltip><template #default="{ row }">{{ row.dependsOn?.join(', ') || '-' }}</template></el-table-column>
    ...
    <el-dialog v-model="dialog.show" :title="dialog.isEdit?'编辑权限组':'新建权限组'" width="650px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="依赖"><el-select v-model="form.dependsOn" multiple filterable placeholder="继承" style="width:100%"><el-option v-for="g in groups" :key="g.id" :label="g.name" :value="g.id" /></el-select></el-form-item>
        <el-divider>规则</el-divider>
        <div v-for="(r, i) in form.rules" :key="i" class="rule-row">
          <el-radio-group v-model="r.effect" size="small"><el-radio-button value="allow">允许</el-radio-button><el-radio-button value="deny">拒绝</el-radio-button></el-radio-group>
...
const groups = ref<PermissionGroup[]>([]); const page = ref(1); const limit = ref(20); const total = ref(0)
const dialog = reactive({ show: false, isEdit: false, editId: '' })
const form = reactive({ name: '', dependsOn: [] as string[], rules: [] as { effect: 'allow'|'deny'; actions: string; priority: number }[] })
function openCreate() { dialog.isEdit=false; dialog.editId=''; form.name=''; form.dependsOn=[]; form.rules=[]; dialog.show=true }
function openEdit(row: PermissionGroup) { dialog.isEdit=true; dialog.editId=row.id; form.name=row.name; form.dependsOn=[...(row.dependsOn||[])]; form.rules=(row.rules||[]).map(r=>({effect:r.effect,actions:r.actions.join(','),priority:r.priority})); dialog.show=true }
...
async function handleSave() {
  ...
  try {
    if (dialog.isEdit) { await api.permissions.apiPermissionsGroupsIdPut(dialog.editId,{rules} as any); ElMessage.success('已更新') }
    else { await api.permissions.apiPermissionsGroupsPost({name:form.name,rules,dependsOn:form.dependsOn||undefined} as any); ElMessage.success('已创建') }
