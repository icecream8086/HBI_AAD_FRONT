...
const items = ref<UserGroup[]>([]); const users = ref<User[]>([]); const page = ref(1); const limit = ref(20); const total = ref(0)
...
onMounted(async () => { await fetchData(); try { users.value = await api.extract<User[]>(api.users.apiUsersGet()) } catch {/* ignore */} })
</script>
