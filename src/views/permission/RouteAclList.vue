  ...
  <div>
    ...
    <el-dialog v-model="dialog.show" :title="dialog.isEdit?'编辑 ACL':'新建 ACL'" width="550px">
      <el-form :model="form" label-width="100px">
        <el-form-item label="方法"><el-select v-model="form.method"><el-option label="GET" value="GET" /><el-option label="POST" value="POST" /><el-option label="PUT" value="PUT" /><el-option label="DELETE" value="DELETE" /><el-option label="*" value="*" /></el-select></el-form-item>
        <el-form-item label="路径前缀"><el-input v-model="form.pathPrefix" placeholder="如: /api/sandboxes" /></el-form-item>
        <el-form-item label="匹配方式"><el-radio-group v-model="form.matchType"><el-radio value="prefix">前缀</el-radio><el-radio value="exact">精确</el-radio></el-radio-group></el-form-item>
        <el-form-item label="效果"><el-radio-group v-model="form.effect"><el-radio value="allow">允许</el-radio><el-radio value="deny">拒绝</el-radio></el-radio-group></el-form-item>
        <el-form-item label="用户/组 ID"><el-input v-model="form.userId" placeholder="用户 ID 或用户组 ID" /></el-form-item>
...
async function fetchData() {
  ...
  catch { ElMessage.error('获取 ACL 失败') }
...
async function handleSave() {
  ...
  try {
    if (dialog.isEdit) { await api.permissions.apiPermissionsRouteAclsIdPut(dialog.editId, p as any); ElMessage.success('已更新') }
    else { await api.permissions.apiPermissionsRouteAclsPost(p as any); ElMessage.success('已创建') }
